...
def computer(data: List[int], start_value: List[int], pointer: int = 0, relative_base: int = 0):
    output = []
...
view = {} # (x, y)
output = computer(data, start_value=None)

x = 0
y = 0
view[(x, y)] = output[4]
while True:
    output = computer(data=output[0], start_value=output[2], pointer=output[1], relative_base=output[3])
    if not output:
        break
...
all_func = [func_A, func_B, func_C, func_draw]
...
while True:
    if output[4] == True:
        print('True')
        output = computer(data=output[0], start_value=all_func.pop(0), pointer=output[1], relative_base=output[3])
    output = computer(data=output[0], start_value=output[2], pointer=output[1], relative_base=output[3])
